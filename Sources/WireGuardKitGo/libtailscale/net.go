...
func (b *backend) updateTUN(rcfg *router.Config, dcfg *dns.OSConfig) error {
	...
	if len(rcfg.LocalAddrs) == 0 {
		b.logger.Logf("updateTUN: skip, no local addr")
		b.appCtx.TunnelClearConfig() // __CYLONIX_MOD__
		return nil
	}
	...
	if err := builder.SetMTU(defaultMTU); err != nil {
		b.logger.Logf("updateTUN: set MTU failed: %v", err)
		return err
	}
	...
	if parcelFD == nil {
		b.logger.Logf("updateTUN: parcelFD is nil")
	...
	if err != nil {
		b.logger.Logf("updateTUN: detachFd failed: %v", err)
		return fmt.Errorf("detachFd: %v", err)
	}
	b.logger.Logf("updateTUN: detached FD")

	// Create TUN device.
	tunDev, _, err := createTUNFromFD(int(tunFD)) // __CYLONIX_MOD__
	if err != nil {
		b.logger.Logf("updateTUN: createTUNFromFD failed: %v. Closing FD.", err)
		b.closeFileDescriptor()
		return err
	}
	...
	b.lastCfg = rcfg
	b.lastDNSCfg = dcfg
	b.tunnelUpdated() // __CYLONIX_MOD__
	return nil
}

func (b *backend) closeFileDescriptor() error {
	if vpnService.fd != -1 && vpnService.fdDetached {
		err := syscall.Close(int(vpnService.fd))
		vpnService.fd = -1
		vpnService.fdDetached = false
		b.logger.Logf("Failed to close file descriptor: %v", err)
...
func (b *backend) getDNSBaseConfig() (ret dns.OSConfig, _ error) {
	...
	log.Printf("getDNSBaseConfig: config=%#v", config)
	return config, nil
}
...
var onTunnelUpdated = make(chan struct{}, 1)
func (b *backend) tunnelUpdated() {
	// Drain any pending update
	select {
    case <-onTunnelUpdated:
        b.logger.Logf("updateTUN: drained pending tunnel update")
    default:
    }

    // Send new update
	onTunnelUpdated <- struct{}{}
    b.logger.Logf("updateTUN: triggered tunnel update")
}

func (b *backend) tunnelUpdatedHandler() {
	name, err := b.devices.Name()
	if err != nil {
		b.logger.Logf("tunnelUpdated: get tunnel name failed: %v", err)
		return
	}
	b.logger.Logf("tunnelUpdated: tunnel name: %s", name)
	ifState := b.netMon.InterfaceState()
	if ifState == nil {
		b.logger.Logf("tunnelUpdated: interface state is nil")
		return
	}
	...
	b.logger.Logf("tunnelUpdated: interface %s not found in state", name)
}
